    ...
    if (!isLocal(attrib_src)) {
        switch (config.image.external.process) {
            ...
            case 'cdn-srcset-when-possible':
                ...
                throw new Error('External images with `cdn-srcset-when-possible` is not implemented yet.');
            case 'add-dimensions-only':
                // Only add dimensions of image
                throw new Error('External images with `add-dimensions-only` is not implemented yet.');
        }
    }
    /*
     * Loading image
